...
def historical_data():
    ...
    plume_all_df["color"] = plume_all_df["plume"].map(
        {"no": "#008000", "yes": "#FF0000"}
    )
    ...
    option = st.selectbox(
        ...
        (
            "Show all locations",
            "Show locations with plume",
            "Show locations without plume",
        ),
    ...
    if option == "Show locations with plume":
        ...
        st.map(
            plume_all_df, use_container_width=True, color="color"
        )  # Display the map with plume = yes
    else:
        st.map(
            plume_no_df, use_container_width=True, color="#008000"
    ...
    location_option = st.selectbox(
        "Choose an option:", unique_cities, label_visibility="collapsed"
    )
    ...
    #     display_city_name_1("Mock data - Paris", mock_data)
        ...
        display_historical_data_for_city(
            location_option,
            cities_data,
            df[["date", "id_coord", "plume", "lat", "lon"]],
        )
